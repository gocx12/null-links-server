...
func (m *defaultTWebsetModel) withSession(session sqlx.Session) *defaultTWebsetModel {
	return &defaultTWebsetModel{
...
func (m *defaultTWebsetModel) Insert(ctx context.Context, data *TWebset) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.Title, data.AuthorId, data.Describe, data.CoverUrl, data.Category, data.ViewCnt, data.LikeCnt, data.FavoriteCnt, data.Status, data.DeletedAt)
	return ret, err
}
...
func (m *defaultTWebsetModel) Update(ctx context.Context, data *TWebset) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, data.Title, data.AuthorId, data.Describe, data.CoverUrl, data.Category, data.ViewCnt, data.LikeCnt, data.FavoriteCnt, data.Status, data.DeletedAt, data.Id)
	return err
}
